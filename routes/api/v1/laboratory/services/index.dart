...
Future<Response> _post(RequestContext context) async {
  final prov = context.read<LaboratoryServicesProviderImpl>();
  final bodyData = await context.request.json() as Map<String, dynamic>;
  LaboratoryServicesModel? serviceModel;
  try {
    serviceModel = LaboratoryServicesModel.fromJson(bodyData);
  } catch (e) {
    return Response.json(
...
Future<Response> _get(RequestContext context) async {
  final param = context.request.uri.queryParameters;
  final prov = context.read<LaboratoryServicesProviderImpl>();
  if (param.isEmpty) {
  ...
  final sedResponse = await prov.getServiceByType(type: param['type']!);
  return Response.json(
    statusCode: int.parse(sedResponse['status_code'] as String),
    body: sedResponse,
  );
